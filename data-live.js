const DATA_LIVE = {
  ...
  "cases": [
    ...
    },
    {
      ...
      "task_ru": "Роман обратился с задачей обновить главную страницу сайта центра сертификации. Компания на рынке более 14 лет, 14 000+ клиентов, 51 000+ оформленных документов - но главная не отражала этот масштаб.\n\nСтарая версия выглядела стандартно: мелкие иконки услуг без живых фотографий, базовый блок \"Почему выбирают нас\" с абстрактными иконками, простой список специалистов без визуального акцента. Текстовые блоки новостей и статей без изображений терялись на странице. Не было промо-акций и явных точек захвата внимания.",
      ...
      "solution_ru": "Дизайнер подготовил макет в Figma, я доработал несоответствия и реализовал вёрстку с интеграцией в WordPress.\n\nHero-блок полностью переработан: живое фото команды вместо стокового, три чётких УТП на видном месте (скидка на оформление, работа без предоплаты, официальное оформление с внесением в реестры). Яркая кнопка CTA сразу ведёт к заявке.\n\nУслуги разделены на два формата. Сверху - популярные запросы с реальными фотографиями сотрудников за работой: декларации, сертификаты, отказные письма. Ниже - полный каталог с фильтрацией по направлениям и отраслям, чтобы клиент быстро нашёл нужную услугу.\n\nДобавлен промо-блок со скидкой -15% при заявке сегодня. Блок \"Узнайте нас\" с ключевыми цифрами компании и фото директора - формирует доверие. Команда получила карточки с должностями и ссылками. Отзывы оформлены в карусель со сканами реальных благодарственных писем.\n\nНовости и статьи переведены из текстовых ссылок в карточки с изображениями. Бегущая строка партнёров расширена новыми логотипами.",
      ...
      "result_ru": "Современная главная страница, которая с первого экрана показывает масштаб и экспертизу компании. Роман проверил результат с коллегами, внесли финальные правки - все остались довольны.\n\nСайт продолжает стабильно держаться в топе поисковой выдачи - SEO-специалист клиента следит за позициями, жалоб после редизайна не было. Продолжаем сотрудничество на постоянной основе: регулярные доработки, новые разделы, реализация сложной логики (включая справочник ТН ВЭД с десятками тысяч записей).",
